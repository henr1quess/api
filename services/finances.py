...
def fetch_all_boletos(
    ...
    if errors:
        st.warning(f"{len(errors)} erro(s) ao buscar boletos. Primeiros 5:")
        for err in errors[:5]:
            st.caption(f"  - {err}")

    df = pd.DataFrame(all_records) if all_records else pd.DataFrame()
    meta = store._save(cache_key, f"financeiro/boletos_consolidado_{ano}", {"ano": ano}, df, TTL_PERMANENT)
    return df, meta
...
def build_financial_dataset(
    ...
    id_aluno_col = _find_column(df, ["id_aluno", "_id_aluno_fetch", "aluno_id", "aluno"])
    valor_nominal_col = _find_column(df, ["valor", "valor_nominal", "valor_titulo", "vlr_nominal"])
    valor_pago_col = _find_column(df, ["valor_pago", "vlr_pago", "valor_recebido"])
    vencimento_col = _find_column(df, ["data_vencimento", "vencimento", "dt_vencimento"])
    pagamento_col = _find_column(df, ["data_pagamento", "dt_pagamento", "data_liquidacao"])
    status_col = _find_column(df, ["status", "situacao", "status_titulo"])
    servico_col = _find_column(df, ["servico", "nome_servico", "descricao_servico", "id_servico"])
    competencia_col = _find_column(df, ["competencia", "mes_competencia", "referencia", "mes_referencia"])

    # ── Join with alunos (nome, turma) ──────────────────────────
    if id_aluno_col and not alunos_df.empty:
        aluno_id_col_alunos = _find_column(alunos_df, ["id", "id_aluno", "aluno_id"])
    ...
    if vencimento_col:
        ...
        if pagamento_col:
            ...
            paid_mask = df[status_col].astype(str).str.lower().isin(paid_statuses)
            df.loc[paid_mask, "dias_atraso"] = 0
...
def detect_inconsistencies(
    ...
    valor_pago_col = _find_column(df, ["valor_pago", "vlr_pago", "valor_recebido"])
    status_col = _find_column(df, ["status", "situacao", "status_titulo"])
    servico_col = _find_column(df, ["servico", "nome_servico", "descricao_servico"])
    nome_col = "nome_aluno" if "nome_aluno" in df.columns else id_aluno_col

    # ── Rule 1: VALOR_DIVERGENTE ─────────────────────────────────
    if valor_nominal_col and valor_pago_col and status_col:
        paid_statuses = ["pago", "liquidado", "recebido", "baixado"]
        mask_paid = df[status_col].astype(str).str.lower().isin(paid_statuses)
        ...
        for _, row in df[mask_diff].iterrows():
            ...
            results.append({
                "id_aluno": row.get(id_aluno_col, ""),
                "nome_aluno": row.get(nome_col, ""),
    ...
    # Enrich nome_aluno from main df if available
    if not result_df.empty and nome_col and nome_col in df.columns and id_aluno_col:
        nome_map = df.set_index(id_aluno_col)[nome_col].drop_duplicates().to_dict()
...
def detect_adverse_operations(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    id_aluno_col = _find_column(df, ["id_aluno", "_id_aluno_fetch", "aluno_id", "aluno"])
    nome_col = "nome_aluno" if "nome_aluno" in df.columns else id_aluno_col
    valor_col = _find_column(df, ["valor", "valor_nominal", "valor_titulo", "vlr_nominal"])
    data_op_col = _find_column(df, [
        "data_operacao", "dt_operacao", "data_registro",
        "data_alteracao", "dt_alteracao", "updated_at",
    ])
    data_pag_col = _find_column(df, ["data_pagamento", "dt_pagamento", "data_liquidacao"])

    # ── 1. CANCELADO ─────────────────────────────────────────────
    if status_col:
        cancel_terms = ["cancelado", "cancelada", "cancel", "estornado", "estornada", "estorno"]
        status_lower = df[status_col].astype(str).str.lower().str.strip()

        mask_cancel = status_lower.isin(["cancelado", "cancelada", "cancel"])
        for _, row in df[mask_cancel].iterrows():
            results.append(_adverse_row(row, "CANCELADO", id_aluno_col, nome_col, valor_col))

        # ── 2. ESTORNADO ─────────────────────────────────────────
        mask_estorno = status_lower.isin(["estornado", "estornada", "estorno"])
...
def build_export_package(
    ...
    if "aging_bucket" in filtered.columns:
        aging = filtered.groupby("aging_bucket", observed=True).agg(
            ...
            valor_total=(_find_column(filtered, ["valor", "valor_nominal", "valor_titulo"]) or "aging_bucket", "count"),
        ).reset_index()
        sheets["Aging"] = aging
...
def mix_forma_recebimento(df: pd.DataFrame) -> pd.DataFrame:
    ...
    forma_col = _find_column(df, ["forma_recebimento", "forma_pagamento", "tipo_recebimento"])
    valor_col = _find_column(df, ["valor", "valor_nominal", "valor_titulo", "vlr_nominal"])
